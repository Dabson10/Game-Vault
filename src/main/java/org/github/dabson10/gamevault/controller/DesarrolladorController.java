...
public class DesarrolladorController {
    ...
    @PostMapping("/create")
    public ResponseEntity<DesarrolladorSimpleDTO> crearDesarrollador(
            @Valid @RequestBody DesarrolladorSimpleDTO des) {
        DesarrolladorSimpleDTO desarrollador = deSe.crearDesarrollador(des);
        return new ResponseEntity<>(desarrollador, HttpStatus.CREATED);
    }

    @GetMapping("/{nombre}/traer")
    public ResponseEntity<DesarrolladorCompletoDTO> buscarDesarrollador(
            @PathVariable @Valid String nombre) {
        DesarrolladorCompletoDTO des = deSe.traerDesarrollador(nombre);
